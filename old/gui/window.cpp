...
void windowobj::render(fonthdl *font)
{
	if (obj == NULL)
		return;
...
void windowobj::select(vec coord)
{
	if (obj != NULL)
	{
		if (type == checkbox_t)
...
void windowobj::move(vec pos, vec delta)
{
	if (obj != NULL)
	{
		if (type == slider_t)
...
void windowhdl::init(const char *n, double a, double b, double w)
{
	strcpy(name, n);
	x = a;
	y = b;
...
void windowhdl::render(fonthdl *font)
{
	if (visible)
	{
		glLoadIdentity();
...
void windowhdl::release()
{
	visible = false;
	windowobj *curr = list, *prev;
	while (curr != NULL)
	{
		prev = curr;
		curr = curr->next;
...
void windowhdl::addobj(void *obj, objecthdl_t type)
{
	windowobj *newobj = new windowobj;
	newobj->obj = obj;
	newobj->type = type;
	newobj->next = NULL;
	newobj->prev = NULL;
...
bool windowhdl::select(vec coord)
{
...
void windowhdl::move(vec pos, vec delta)
{
	if (bar_selected)
	{
		x += delta.x;
...
bool windowhdl::mousehover(vec coord)
{
