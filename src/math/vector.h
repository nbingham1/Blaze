...
struct vec
{
	...
	{
		vec<ref<t>, s> result;
		for (int i = start; i <= minimum(s, end); i++)
...
struct vec<float, s>
{
	...
	/* print
	 *
	 * Prints this vector's components to a file in the form of
	 * [ x y z w ... ]
	 */
	void print(FILE *file)
	{
	...
	/* print
	 *
	 * Prints this vector's components to stdout in the form of
	 * [ x y z w ... ]
	 */
	void print()
	{
...
template <class t, class t2, int s>
vec<t, (s+1)> operator, (vec<t, s> v, t2 a)
{
	vec<t, (s+1)> result;
	for (int i = 0; i < s; i++)
		result.data[i] = v.data[i];
...
vec<t, s + s2> operator, (vec<t, s> v1, vec<t2, s2> v2)
{
	vec<t, s + s2> result(v1);
	for (int i = s; i < s + s2; i++)
		result.data[i] = v2.data[i-s];
	return result;
}
