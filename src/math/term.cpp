...
term::term(string s)
{
	...
	coeff = s.substr(0, x).to_float();
	a = s.substr(x+1, y-x-1).to_int();
	b = s.substr(y+1, z-y-1).to_int();
	c = s.substr(z+1, w-z-1).to_int();
	d = s.substr(w+1).to_int();
}
